<script lang="ts">
  ...
  const DEFAULTS = {
    ...
    T1: 530, Q: 645,
    eps_regen: 90, regen_dP_pct: 3.5,
    eta_turb: 0.95, eta_comp: 0.90, eta_gen: 0.985,
    T0: 25, RH: 50, cw_approach: 3.1, pc_approach: 18.9, itc_approach: 18.9, cw_range: 10,
  };
  ...
  let eps_regen = $state(DEFAULTS.eps_regen);
  let regen_dP_pct = $state(DEFAULTS.regen_dP_pct);
  let eta_turb  = $state(DEFAULTS.eta_turb);
  ...
  function getChain(): Chain {
    return [
      ['P_high', () => P_high, v => P_high = snap(v), 3.5, 7.0],
      ['P_mid',  () => P_mid,  v => P_mid  = snap(v), 2.5, 5.0],
      ['P_low',  () => P_low,  v => P_low  = snap(v), 1.5, 3.2],
    ];
  }
  ...
  function enforceOrder(changed: string) {
    ...
    if (clamped) {
      flagOrder('Adjacent pressure(s) shifted to keep the loop ordered (compressor discharge > intercooler > turbine exhaust).');
    }
  }
  ...
  function params() {
    return {
      ...
      T1, Q,
      eps_regen: eps_regen / 100, regen_dP_pct: regen_dP_pct / 100,
      eta_turb, eta_comp, eta_gen,
      T0, RH, cw_approach, pc_approach, itc_approach, cw_range,
    };
  }
  ...
  async function runSolve() {
    try {
      result = await solveHEAsync(params());
      errMsg = null;
  ...
  function resetAll() {
    ...
    T1 = DEFAULTS.T1; Q = DEFAULTS.Q;
    eps_regen = DEFAULTS.eps_regen; regen_dP_pct = DEFAULTS.regen_dP_pct;
    eta_turb = DEFAULTS.eta_turb; eta_comp = DEFAULTS.eta_comp; eta_gen = DEFAULTS.eta_gen;
    T0 = DEFAULTS.T0; RH = DEFAULTS.RH; cw_approach = DEFAULTS.cw_approach;
    pc_approach = DEFAULTS.pc_approach; itc_approach = DEFAULTS.itc_approach; cw_range = DEFAULTS.cw_range;
    orderWarning = '';
    if (warnTimeout) clearTimeout(warnTimeout);
    errMsg = null;
    runSolve();
  ...
  const g3Warn = $derived(!!result && result.bwr > 0.6);
  ...
  const S_LO = 19, S_HI = 26, T_LO = 20, T_HI = 560;
  ...
  const T_TICKS = [50, 150, 250, 350, 450, 550];
  const S_TICKS = [19, 20, 21, 22, 23, 24, 25, 26];
...
<div class="page-header">
  <h1 class="page-title">Supercritical He Brayton Cycle</h1>
  ...
  {:else}
    <div class="controls-col">
      ...
      <div class="slider-details chamfer-panel chamfer-sm" class:slider-open={openSections.gas} style="--slider-color: #e8935f; --slider-tint: rgba(232, 147, 95, 0.2); --slider-glow-1: rgba(232, 147, 95, 0.55); --slider-glow-2: rgba(232, 147, 95, 0.85)">
        ...
        <div class="slider-body" transition:slide={{ duration: 200 }}>
          <div class="slider-row">
            <div class="slider-label"><span>Aux compressor (C2) discharge P</span><span class="slider-value">{P_high} MPa</span></div>
            <input id="h-phigh" type="range" min="3.5" max="7.0" step="0.05" bind:value={P_high} style="--pct: {pct(P_high,3.5,7.0)}%" onchange={() => onPressureChange('P_high')} />
          </div>
          <div class="slider-row">
            <div class="slider-label"><span>Main compressor (C1) discharge P</span><span class="slider-value">{P_mid} MPa</span></div>
            <input id="h-pmid" type="range" min="2.5" max="5.0" step="0.05" bind:value={P_mid} style="--pct: {pct(P_mid,2.5,5.0)}%" onchange={() => onPressureChange('P_mid')} />
          </div>
          <div class="slider-row">
            <div class="slider-label"><span>Turbine exhaust P (loop low side)</span><span class="slider-value">{P_low} MPa</span></div>
            <input id="h-plow" type="range" min="1.5" max="3.2" step="0.02" bind:value={P_low} style="--pct: {pct(P_low,1.5,3.2)}%" onchange={() => onPressureChange('P_low')} />
          </div>
          <div class="slider-row">
            <div class="slider-label"><span>Turbine inlet T₁</span><span class="slider-value">{T1} °C</span></div>
            <input id="h-t1" type="range" min="400" max="600" step="5" bind:value={T1} onchange={runSolve} />
          </div>
          <div class="slider-row">
            <div class="slider-label"><span>Secondary HTX duty Q</span><span class="slider-value">{Q} MW</span></div>
            <input id="h-q" type="range" min="200" max="2000" step="5" bind:value={Q} onchange={runSolve} />
          </div>
        </div>
        {/if}
      </div>

      <div class="slider-details chamfer-panel chamfer-sm" class:slider-open={openSections.regen} style="--slider-color: #1a9b73; --slider-tint: rgba(26, 155, 115, 0.2); --slider-glow-1: rgba(26, 155, 115, 0.55); --slider-glow-2: rgba(26, 155, 115, 0.85)">
        <button type="button" class="details-summary" aria-expanded={openSections.regen} onclick={() => openSections.regen = !openSections.regen}>
          Regeneration
          <svg class="chevron" viewBox="0 0 10 10" width="10" height="10">
            <polyline points={chevronPoints(openSections.regen)} class="chevron-bg" />
            <polyline points={chevronPoints(openSections.regen)} class="chevron-fg" />
          </svg>
        </button>
        {#if openSections.regen}
        <div class="slider-body" transition:slide={{ duration: 200 }}>
          <div class="slider-row">
            <div class="slider-label"><span>Regenerator effectiveness</span><span class="slider-value">{eps_regen} %</span></div>
            <input id="h-eregen" type="range" min="50" max="99" step="0.5" bind:value={eps_regen} onchange={runSolve} />
          </div>
          <div class="slider-row">
            <div class="slider-label"><span>Regenerator pressure drop</span><span class="slider-value">{regen_dP_pct} %</span></div>
            <input id="h-eregdp" type="range" min="0" max="6" step="0.1" bind:value={regen_dP_pct} onchange={runSolve} />
          </div>
          <p class="cond-helper">
            A single regenerator here (not split into high-/low-temperature stages like the
            CO<sub>2</sub> cycle) - helium's near-ideal-gas cp barely varies with temperature,
            so there's no sharp mismatch between the hot and cold streams to balance. The pressure
            drop applies to both sides (as a % of each side's own local pressure) - it's the one
            heat-exchanger pressure loss this simulator models, chosen because the main
            compressor's inlet pressure turns out to be unusually sensitive to it.
          </p>
        </div>
        {/if}
      </div>
      ...
      <div class="slider-details chamfer-panel chamfer-sm" class:slider-open={openSections.efficiencies} style="--slider-color: #a06cd5; --slider-tint: rgba(160, 108, 213, 0.2); --slider-glow-1: rgba(160, 108, 213, 0.55); --slider-glow-2: rgba(160, 108, 213, 0.85)">
        ...
        <div class="slider-body" transition:slide={{ duration: 200 }}>
          <div class="slider-row">
            <div class="slider-label"><span>η turbine</span><span class="slider-value">{(eta_turb*100).toFixed(0)} %</span></div>
            <input id="h-etat" type="range" min="0.5" max="1" step="0.01" bind:value={eta_turb} onchange={runSolve} />
          </div>
          <div class="slider-row">
            <div class="slider-label"><span>η compressors (both)</span><span class="slider-value">{(eta_comp*100).toFixed(0)} %</span></div>
            <input id="h-etac" type="range" min="0.5" max="1" step="0.01" bind:value={eta_comp} onchange={runSolve} />
          </div>
          <div class="slider-row">
            <div class="slider-label"><span>η generator</span><span class="slider-value">{(eta_gen*100).toFixed(1)} %</span></div>
            <input id="h-gen" type="range" min="0.95" max="1" step="0.001" bind:value={eta_gen} onchange={runSolve} />
          </div>
        </div>
        {/if}
      </div>
      ...
      <div class="slider-details chamfer-panel chamfer-sm" class:slider-open={openSections.cooling} style="--slider-color: #5ba3e8; --slider-tint: rgba(91, 163, 232, 0.2); --slider-glow-1: rgba(91, 163, 232, 0.55); --slider-glow-2: rgba(91, 163, 232, 0.85)">
        ...
        <div class="slider-body" transition:slide={{ duration: 200 }}>
          <div class="slider-row">
            ...
            <input id="h-t0" type="range" min="0" max="40" step="1" bind:value={T0} onchange={runSolve} />
          </div>
          <div class="slider-row">
            <div class="slider-label"><span>Relative humidity</span><span class="slider-value">{RH} %</span></div>
            <input id="h-rh" type="range" min="10" max="99" step="1" bind:value={RH} onchange={runSolve} />
          </div>
          <div class="slider-row">
            <div class="slider-label"><span>Cooling tower approach</span><span class="slider-value">{cw_approach} °C</span></div>
            <input id="h-cwa" type="range" min="2" max="15" step="0.1" bind:value={cw_approach} onchange={runSolve} />
          </div>
          <div class="slider-row">
            <div class="slider-label"><span>Precooler approach</span><span class="slider-value">{pc_approach} °C</span></div>
            <input id="h-pca" type="range" min="3" max="30" step="0.1" bind:value={pc_approach} onchange={runSolve} />
          </div>
          <div class="slider-row">
            <div class="slider-label"><span>Intercooler approach</span><span class="slider-value">{itc_approach} °C</span></div>
            <input id="h-ica" type="range" min="3" max="30" step="0.1" bind:value={itc_approach} onchange={runSolve} />
          </div>
          <div class="slider-row">
            <div class="slider-label"><span>CW temperature rise</span><span class="slider-value">{cw_range} °C</span></div>
            <input id="h-cwr" type="range" min="5" max="20" step="0.5" bind:value={cw_range} onchange={runSolve} />
          </div>
          {#if result}
            <p class="cond-helper">
              Main compressor intake: {fmt(result.T4, 1)} °C · Aux compressor intake: {fmt(result.T6, 1)} °C ·
              cooling tower basin {fmt(result.T_cw_in, 1)} °C
            </p>
          {/if}
        </div>
        {/if}
    ...
    <div class="diagram-col">
      ...
      <svg viewBox="0 0 {SVG_W} {SVG_H}" class="ts-svg" role="img"
           aria-label="T-s diagram of the recompression supercritical He Brayton cycle">
        ...
        {#if result}
          ...
          <path d={pathD(r.shxPath)} class="path-shx" />
          <path d={pathD(r.turbPath)} class="path-expand" />
          <path d={pathD(r.regenHotPath)} class="path-regen-hot" />
          <path d={pathD(r.precoolPath)} class="path-precool" />
          <path d={pathD(r.c1Path)} class="path-comp" />
          <path d={pathD(r.itcPath)} class="path-itc" />
          <path d={pathD(r.c2Path)} class="path-comp-aux" />
          <path d={pathD(r.regenColdPath)} class="path-regen-cold" />
      ...
      <div class="diagram-legend">
        ...
        <span class="leg leg-expand">Turbine</span>
        ...
        <span class="leg leg-itc">Intercooler</span>
      ...
      {#if result}
        ...
        <div class="readout-grid">
          ...
          <div class="readout-card chamfer-panel chamfer-sm">
            <p class="readout-label">Intercooler duty</p>
            <p class="readout-value">{fmt(result.Q_itc / 1e6, 1)} <span class="readout-unit">MW</span></p>
          </div>
          <div class="readout-card chamfer-panel chamfer-sm">
            <p class="readout-label">Regenerator duty</p>
            <p class="readout-value">{fmt(result.Q_regen / 1e6, 1)} <span class="readout-unit">MW</span></p>
          </div>
          <div class="readout-card chamfer-panel chamfer-sm">
            <p class="readout-label">Main comp. intake temp</p>
            <p class="readout-value">{fmt(result.T4, 1)} <span class="readout-unit">°C</span></p>
          </div>
          <div class="readout-card chamfer-panel chamfer-sm">
            <p class="readout-label">Aux comp. intake temp</p>
            <p class="readout-value">{fmt(result.T6, 1)} <span class="readout-unit">°C</span></p>
          </div>
          <div class="readout-card chamfer-panel chamfer-sm">
...
<style>
  ...
  /* Independent scroll panes - see RankineCycle.svelte / CO2BraytonCycle.svelte
     for the full rationale. Colored scrollbar per column (amber left, teal
     right) plus a vertical seam down the gap are the visual tell that these
     are two separate scroll regions, not one long page. */
  ...
  .path-itc        { fill: none; stroke: #2fb8c4; stroke-width: 2; }
  ...
  .leg-itc::before        { background: #2fb8c4; }
